...
async function fetchJson<T>(
  ...
  try {
    ...
    if (!Value.Check(schema, body)) {
      debug('GET %s returned unexpected shape', url)
      return undefined
    }
    return body as T
  } catch (err) {
...
export function createRegistryClient(
  ...
  function readPluginFromDisk(name: string): PluginEntry | undefined {
    ...
    try {
      ...
      debug('readPlugin failed for %s: %O', name, err)
      return undefined
    }
  }
  ...
  function writePluginToDisk(name: string, payload: RegistryPluginDetail) {
    ...
    try {
      ...
      debug('writePlugin failed for %s: %O', name, err)
    }
  }
    ...
    invalidate() {
      ...
      if (indexFile && fs.existsSync(indexFile)) {
        try {
          ...
          debug('invalidate index failed: %O', err)
        }
      }
    },
