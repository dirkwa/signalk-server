...
export function createIconBytesCache(cacheDir: string): IconBytesCache {
  ...
  function ensureRoot() {
    try {
      ...
      debug('mkdir %s failed: %O', root, err)
    }
  }
    ...
    try {
      ...
      for (const entry of fs.readdirSync(root)) {
        ...
        try {
          ...
          debug('purged stale icon %s', entry)
        } catch (err) {
          debug('purge %s failed: %O', entry, err)
        }
      }
    } catch (err) {
      debug('purgeOldVersions %s failed: %O', pkgName, err)
    }
  }
  ...
  function findStored(pkgName: string): StoredIcon | undefined {
    ...
    try {
      ...
      debug('findStored %s failed: %O', pkgName, err)
      return undefined
    }
  ...
  async function downloadBytes(
    ...
    try {
      ...
      if (!res.ok) {
        debug('GET %s returned %d', cdnUrl, res.status)
        return null
      }
      ...
      debug('GET %s failed: %O', cdnUrl, err)
      return null
    }
    ...
    try {
      ...
      debug('write %s failed: %O', full, err)
      return null
    }
  }
