...
async function fetchJson<T>(url: string): Promise<T | undefined> {
  try {
    ...
    if (!res.ok) {
      debug('GET %s returned %d', url, res.status)
      return undefined
    }
    return (await res.json()) as T
  } catch (err) {
    debug('GET %s failed: %O', url, err)
    return undefined
  }
}
...
async function fetchGithubRepo(
  owner: string,
  repo: string
): Promise<{ stars?: number; openIssues?: number } | undefined> {
...
async function fetchContributorCount(
  ...
  try {
    ...
    debug('contributors fetch failed: %O', err)
    return undefined
  }
}
...
export function createRawMetricsClient(cacheDir: string): RawMetricsClient {
  ...
  function ensureDir() {
    try {
      ...
      debug('mkdir %s failed: %O', dir, err)
    }
  }
  ...
  return {
    async get(pkgName, githubUrl) {
      ...
      // Kick off all three in parallel. Each independently tolerates
