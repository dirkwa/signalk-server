 ...
 * `localBluetoothManaged` setting is enabled (default: true).
 */
...
export class LocalBLEProvider {
  ...
  availableGATTSlots(): number {
    return Math.max(0, this.maxGATTSlots - this.sessions.size)
  }
  ...
  private async attachDeviceListener(mac: string) {
    try {
      ...
      // Read initial properties and emit
      this.emitDeviceAdvertisement(device, mac)

      // Listen for property changes (new advertisements)
  ...
  private async emitDeviceAdvertisement(
    ...
    try {
      ...
      if (changedProps) {
        // Incremental update from PropertiesChanged
      ...
      // Ignore errors reading device properties
    }
  }
  ...
  private async executeGATTLifecycle(session: LocalGATTSession) {
    ...
    // 9. Set up disconnect handler for auto-reconnect
    session.device.on('disconnect', () => {
      ...
      session.reconnectTimer = setTimeout(async () => {
        if (session.closed) return
        debug(`GATT reconnecting to ${mac}`)
        try {
          await this.executeGATTLifecycle(session)
          ...
          // Will retry on next disconnect or can be manually retried
        }
      }, 5000)
    })
  ...
  async connectGATT(mac: string): Promise<BLEGattConnection> {
    ...
    const device = await this.connectQueue.enqueue(async () => {
