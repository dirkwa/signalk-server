...
describe('appstore/indicators', () => {
  ...
  it('marks unknown signals as ok with "unable to determine" subtitle', () => {
    const r = computeIndicators({})
    const testsCheck = r.checks.find((c) => c.id === 'tests-pass')
    expect(testsCheck?.status).to.equal('ok')
  ...
  it('does not compute reportedPlatforms automatically', () => {
    const r = computeIndicators({})
    expect(r.reportedPlatforms).to.deep.equal([])
  })
})
