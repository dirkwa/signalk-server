...
describe('appstore detail cache', function () {
  ...
  async function anAvailablePlugin(
    ...
    if (!candidate) {
      // loadPluginDetail only reaches the cache short-circuit for a name
      // the npm search returned, and that search has no offline fallback,
      // so with no registry there is no match to short-circuit on. Skip
      // loudly rather than silently reporting a pass.
      console.warn(
        'appstore detail cache: npm search returned no packages ' +
          '(storeAvailable=%s) — skipping, cache behaviour NOT verified',
        body.storeAvailable
      )
      ctx.skip()
    }
