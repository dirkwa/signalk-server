...
import { Card, CardBody, Progress, ListGroup, ListGroupItem, Spinner, Alert } from 'reactstrap'
  ...
  useEffect(() => {
    ...
    const startInstall = async () => {
      ...
      try {
        ...
        unlisten = await listen<{ step: string; status: string; message?: string }>(
          'install-progress',
          (event) => {
            setSteps((prev) =>
              prev.map((step) =>
          ...
          if (errorStr.includes('invoke')) {
            ...
            for (let i = 0; i < initialSteps.length; i++) {
              ...
              setSteps((prev) =>
                prev.map((step, idx) => ({
                  ...step,
                  status: idx <= i ? 'completed' : idx === i + 1 ? 'in_progress' : 'pending',
                }))
              )
            }
            setError(null)
